...
var getItems = function(callback){
    ...
    pricePoints_new_fixed.findByKeyword('Playstation 4',function(documents){
        ...
        ebay.findByKeyword(ps4.new_keyword_fixed,documents[0]['50percent'],documents[0]['40percent']
...
var calcShipping = function(){
    ...
    items_new_fixed.findCalcShipping(function(items){
        if(items.length == 0){
...
db.connect('MONGO_URL', function(err){
  if(err) {
    ...
    schedule.scheduleJob({hour:7,minute:30},function(){
        ebay.findCompletedKeyword(ps4.new_completed_fixed,90,380,ps4.addedKeywordFilters);
    })
    ...
    schedule.scheduleJob({hour:7,minute:40}, function(){
        items_new_fixed.getYesterdays(function(yItems){
            if(yItems.length == 0){
