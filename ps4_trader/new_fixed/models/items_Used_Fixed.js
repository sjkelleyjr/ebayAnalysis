...
exports.insert = function(item, callback) {
  var collection = db.get().collection('items_used_fixed')
  collection.insert(item, function(err, result) {
    assert.equal(err, null)
    assert.equal(1, result.result.n)
    assert.equal(1, result.ops.length)
    console.log('Inserted 1 document into the items_used_fixed collection')
    console.log(item)
    callback(result)
  })
}
...
exports.find = function(id,callback){
  ...
  collection.findOne({'_id':id}, function(err, document) {
    assert.equal(err, null)
    console.log('Found 1 items_used_fixed document')
    callback(document)
  })
}
...
exports.update = function(item,callback){
  ...
  collection.update({'_id': item._id},item, function(err, result) {
    assert.equal(err, null)
    assert.equal(1, result.result.n)
    console.log('updated 1 items_used_fixed document');
    console.log(item);
    callback(result);
  })
}
...
exports.remove = function(item,callback){
  ...
  collection.remove({'_id':item._id}, function(err, document) {
    assert.equal(err, null)
    if(document){
      console.log('Removed 1 items_used_fixed document')
    }
    callback(document)
  })
}
...
exports.addTag = function(itemId,tag,callback){
  ...
  collection.update(
    ...
    function(err, result) {
      ...
      assert.equal(1, result.result.n)
      // assert.equal(1, result.ops.length)
      console.log('pushed 1 tag to the items_used_fixed')
      callback(result)
  })
}
