...
exports.insert = function(item, callback) {
  var collection = db.get().collection('pricePoints_used_auction')
  collection.insert(item, function(err, result) {
    assert.equal(err, null)
    assert.equal(1, result.result.n)
    assert.equal(1, result.ops.length)
    console.log('Inserted 1 document into the pricePoints_used_auction collection')
    console.log(item)
    callback(result)
  })
}
...
exports.find = function(id,callback){
  ...
  collection.findOne({'_id':id}, function(err, document) {
    assert.equal(err, null)
    console.log('Found 1 pricePoints_used_auction document')
    callback(document)
  })
}
...
exports.updateKeyword = function(item,callback){
  ...
  collection.update({'keywords': {$in:item.keywords}},item, function(err, result) {
    assert.equal(err, null)
    assert.equal(1, result.result.n)
    console.log('updated 1 pricePoints_used_auction document');
    console.log(item);
    callback(result);
  })
}
...
exports.addTag = function(itemId,tag,callback){
  ...
  collection.update(
    ...
    function(err, result) {
      ...
      assert.equal(1, result.result.n)
      // assert.equal(1, result.ops.length)
      console.log('pushed 1 pricePoints_used_auction to the item')
      callback(result)
  })
}
