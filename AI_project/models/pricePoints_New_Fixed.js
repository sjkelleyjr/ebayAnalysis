...
exports.insert = function(item, callback) {
  var collection = db.get().collection('pricePoints_new_fixed')
  collection.insert(item, function(err, result) {
    assert.equal(err, null)
    assert.equal(1, result.result.n)
    assert.equal(1, result.ops.length)
    console.log('Inserted 1 document into the pricePoints_new_fixed collection')
    console.log(item)
    callback(result)
  })
}
...
exports.find = function(id,callback){
  ...
  collection.findOne({'_id':id}, function(err, document) {
    assert.equal(err, null)
    console.log('Found 1 pricePoints_new_fixed document')
    callback(document)
  })
}
...
exports.update = function(user,callback){
  ...
  collection.update({'_id': item._id},item, function(err, result) {
    assert.equal(err, null)
    assert.equal(1, result.result.n)
    console.log('updated 1 pricePoints_new_fixed document');
    console.log(item);
    callback(result);
  })
}
...
exports.addTag = function(itemId,tag,callback){
  ...
  collection.update(
    ...
    function(err, result) {
      ...
      assert.equal(1, result.result.n)
      // assert.equal(1, result.ops.length)
      console.log('pushed 1 pricePoints_new_fixed to the item')
      callback(result)
  })
}
